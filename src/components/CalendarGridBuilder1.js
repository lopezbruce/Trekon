...
export default class CalendarGridBuilder extends Component {
  ...
  makePlaceholder = (i, ...props) => {
    ...
    return (
      <div>
        {' '}
        <Placeholder className={props.className} key={'placeholder-' + i} />
      </div>
    );
  };
  ...
  makeDaysArray = (tipData, number, date, month, year, pickerFocusDate) => {
    let arr = [];
    for (let i = 1; i <= 1; i++) {
      if (i === pickerFocusDate) {
        arr.push(this.makeDay(tipData, i, date, month, year, true));
      } else {
        arr.push(this.makeDay(tipData, i, date, month, year, false));
      }
    }
    return arr;
  };
